...
require(["jquery"], function ($) {
  $('[data-toggle="collapse"][data-target]').on('click', function (evt) {
  ...
  $('[data-action="selectStation"]').on('change', function () {
    ...
    jQuery('option', 'select[name="tx_c3baxi_web_c3baxibaxi[' + selectName + ']"]').attr('disabled', false).filter('[data-zone="' + selectedZone + '"]').attr('disabled', true);
    $.ajax({
      ...
      var tmpl = $('#boxTemplate')[0].innerHTML,
          html = '';
      html = tmpl.replace(/##name##/g, response.name).replace(/##zone##/g, response.zone).replace(/##linie##/g, response.linie.join(','));
      target.html(html);
    });
  });
  ...
  function createRideList(response) {
    if (!response.status) {
      var error = jQuery('<div class="error">').text('Es ist ein Fehler aufgetreten!');
      jQuery('.output').append(error);
      return false;
    }

    if (response.status === 'not_found') {
      var _error = jQuery('<div class="error">').text('Es wurden keine Fahrten gefunden!');

      jQuery('.output').append(_error);
      return false;
    }

    var output = jQuery('.list', '.output'),
        data = response.data,
    ...
    for (var r = 0; r < list.length; r++) {
      ...
      ride = ride.replace(/##line##/g, list[r].linie).replace(/##duration##/g, list[r].dauer).replace(/##departure##/g, departureStr).replace(/##arrival##/g, arrivalStr).replace(/##date##/g, rideDate.toLocaleDateString('de-DE')).replace(/##time##/g, bookingTimeStr);
      ride = jQuery(ride);
